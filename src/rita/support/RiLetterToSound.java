 ...
 */
 /* <p>
 * The LTS rules are a simple state machine, with one entry point for each
 * letter of the alphabet (lower case letters are always assumed, and the rules
 * keep an array with one entry per letter that point into the state machine).
 * 
 * <p>
 * The state machine consists of a huge array, with most entries containing a
 * decision and the indeces of two other entries. The first of these two indeces
 * represents where to go if the decision is true, and the second represents
 * where to go if the decision is false. All entries that do not contain a
 * decision are final entries, and these contain a phone.
 * 
 * <p>
 * The decision in this case is a simple character comparison, but it is done in
 * the context of a window around the character in the word. The decision
 * consists of a index into the context window and a character value. If the
 * character in the context window matches the character value, then the
 * decision is true.
 * 
 * <p>
 * The machine traversal for each letter starts at that letter's entry in the
 * state machine and ends only when it reaches a final state. If there is no
 * phone that can be mapped, the phone in the final state is set to 'epsilon.'
 * 
 * <p>
 * The context window for a character is generated in the following way:
 * 
 * <ul>
 * <li>Pad the original word on either side with '#' and '0' characters the
 * size of the window for the LTS rules (in this case, the window size is 4).
 * The "#" is used to indicate the beginning and end of the word. So, the word
 * "monkey" would turn into "000#monkey#000".
 * <li>For each character in the word, the context window consists of the
 * characters in the padded form the preceed and follow the word. The number of
 * characters on each side is dependent upon the window size. So, for this
 ...
 * <li>Grab the 'index' from the current state. This represents an index into
 * the context window.
 * <li>Compare the value of the character at the index in the context window to
 * the character from the current state. If there is a match, the next state is
 * the qtrue value. If there isn't a match, the next state is the qfalse state.
 * <li>Keep on working through the machine until you read a final state.
 * <li>When you get to the final state, the phone is the character in that
 * state.
 * </ul>
 * 
 * <p>
 * This implementation will either read from a straight ASCII file or a binary
 * file. When reading from an ASCII file, you can specify when the input line is
 * tokenized: load, lookup, or never. If you specify 'load', the entire file
 * will be parsed when it is loaded. If you specify 'lookup', the file will be
 * loaded, but the parsing for each line will be delayed until it is referenced
 * and the parsed form will be saved away. If you specify 'never', the lines
 * will parsed each time they are referenced. The default is 'load'. To specify
 * the load type, set the system property as follows:
 * 
 * <pre>
 ...
 * 
 * <p>
 * [[[TODO: This implementation uses ASCII 'a'-'z', which is not
 * internationalized.]]]
 */
public class RiLetterToSound implements LetterToSound
{
  /**
  ...
  public RiLetterToSound(URL ltsRules) throws IOException {
    this(ltsRules, false);      
  }
   ...
   * 
   * @param ltsRules
   *          a URL pointing to the text containing the letter to sound rules
   * @param binary
   ...
   * @throws IOException
   *           if errors are encountered while reading the compiled form or the
   *           addenda
   */
  public RiLetterToSound(URL ltsRules, boolean binary) throws IOException {
    //System.out.println("RiLetterToSound.RiLetterToSound("+ltsRules+")");
  ...
  private void loadText(InputStream is) throws IOException
  {
    ...
    String tokenize = Utilities.getProperty(
        "com.sun.speech.freetts.lexicon.LTSTokenize", "load");
  ...
  /**
   * Loads the LTS rules from the given binary input stream. The input stream is
   * not closed after the rules are read.
   * 
   * @param is
   *          the input stream
   * 
  ...
  /**
   * Dumps a binary form of the letter to sound rules. This method is not
   * thread-safe.
   * 
   * <p>
   * Binary format is:
  ...
  public String[] getPhones(String word, String partOfSpeech)
  {
    ...
    for (int pos = 0; pos < word.length(); pos++)
    {
      ...
      if (startIndex == null)
        throw new RiTaException("Null startIndex!");
  ...
  public boolean compare(RiLetterToSound other)
  {
    ...
    for (Iterator i = letterIndex.keySet().iterator(); i.hasNext();)
    {
      ...
      {
        if (VERBOSE)
          System.out.println("[WARN] Bad Index for " + key);
        return false;
      }
    }
    ...
    for (int i = 0; i < stateMachine.length; i++)
    {
      ...
      {
        if (VERBOSE)
          System.out.println("[WARN] Bad state " + i);
        return false;
      }
    }
  ...
  /**
   * A <code>State</code> that represents a final state in the state machine.
   * It contains one or more phones from the phone table.
   * 
   * @see DecisionState
   */
      ...
      if (args.length > 0)
      {
        ...
        for (int i = 0; i < args.length; i++)
        {
          ...
          else if (args[i].equals("-generate_binary"))
          {
            ...
            RiLetterToSound text = new RiLetterToSound(new URL("file:"
                + srcPath + "/" + name + ".txt"), false);
          ...
          else if (args[i].equals("-compare"))
          {
            ...
            timer.start("load_text");
            RiLetterToSound text = new RiLetterToSound
              (new URL("file:./" + name + ".txt"), false);
            timer.stop("load_text");

            timer.start("load_binary");
            RiLetterToSound binary = new RiLetterToSound
