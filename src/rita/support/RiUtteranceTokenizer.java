...
public class RiUtteranceTokenizer implements Tokenizer {
  ...
  // a place to store the current and last tokens
  private Token token, lastToken;
   ...
   */
  public RiUtteranceTokenizer(String string) {
    setInputText(string);
  }
   ...
   */
  public RiUtteranceTokenizer(Reader file) {
    setInputReader(file);
  }
   ...
   * subsequent characters of type charClass, and not of type singleCharSymbols.
   * 
   * @param charClass
  ...
  /**
   * Removes the postpunctuation characters from the current token. Copies those
   * postpunctuation characters to the class variable 'postpunctuation'.
   */
  private void removeTokenPostpunctuation() {
    if (token != null) {
      ...
      while (position > 0
          && postpunctuationSymbols.indexOf((int) tokenWord.charAt(position)) != -1) {
        position--;
      }
  ...
  public boolean isBreak() {
    ...
    char first = token.getWord().charAt(0);
    String tokenWhiteSpace = token.getWhitespace();
    String lastTokenPostpunctuation = lastToken.getPostpunctuation();
    ...
    if (tokenWhiteSpace.indexOf('\n') != tokenWhiteSpace.lastIndexOf('\n')) 
    {
      return true;
        ...
        (lastTokenPostpunctuation.indexOf('?') != -1) && Character.isUpperCase(first)) 
            //&& token.is
