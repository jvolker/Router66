 ...
 * @see VoiceDirectory
 */
public class RiVoiceManager {

    CMUDiphoneVoice defaultVoice;
    
    private static RiVoiceManager instance;
    private static String pathSeparator = RiTa.OS_SLASH;
    ...
    private RiVoiceManager() {
      try {
        classLoader = new DynamicClassLoader(new URL[0]);       
      } 
      catch (Throwable e) {
        // ignore
      }
      if (classLoader == null) {
        if (DBUG_VOICE_LOADING)
          System.out.println("[WARN] RiVoiceManager has null classLoader!");
        //classLoader = RiVoiceManager.class.getClassLoader();
      }
    }
     ...
     */
    public static RiVoiceManager getInstance() {
      if (instance == null) 
        instance = new RiVoiceManager();
      return instance;
    }
    ...
    public Voice[] getVoices() {
        ...
        if (voiceDirectories==null) return null;
        for (int i = 0; i < voiceDirectories.length; i++) {
//System.out.println(i+") "+voiceDirectories[i]);
    ...
    {      
      try 
      {
          ...
          if (DBUG_VOICE_LOADING) System.out.println("TEST: got " +
          ...
          voiceDirectoryNames.addVector(
           ...
           if (DBUG_VOICE_LOADING) 
             for (int i = 0; i < voiceDirectoryNames.size(); i++) 
               System.out.println("TEST VoiceDir: " + voiceDirectoryNames.get(i));
          ...
          return (VoiceDirectory[]) voiceDirectories.toArray
            (new VoiceDirectory[voiceDirectories.size()]);
      }       
      catch (SecurityException se) {
        if (DBUG_VOICE_LOADING) System.err.println
          ("[WARN] Security problem in getVoiceDirectories()!\n"+se.getMessage());
        return null;
      }
    ...
    static boolean DBUG_VOICE_LOADING = false;
...
//System.out.println("[INFO] Loading voices from internal_voices.txt");
              voiceDirectoryNames.addVector(getVoiceDirectoryNamesFromInputStream(is));
            }
//else System.out.println("[INFO] Unable to load voices from internal_voices.txt");
            ...
            String voicesFile=null;
            try {
              voicesFile = System.getProperty("freetts.voicesfile");
            } catch (Exception e) {
              // ignore
            }
            if (voicesFile != null) 
    ...
    private UniqueVector getVoiceJarURLs() {
        ...
        try {
            ...
            if (!baseDirectory.equals("")) {  // not called from a jar
              UniqueVector v = getVoiceJarURLsFromDir(baseDirectory);
              if (v!=null)
              voiceJarURLs.addVector(v);
            }
        ...
        String voicesPath="";
        try {
          voicesPath = System.getProperty("freetts.voicespath", "");
        } catch (Exception e1) {
          // ignore
        }
        ...
        if (DBUG_VOICE_LOADING)
          System.out.println("voiceJarURLs="+voiceJarURLs);
    ...
    private UniqueVector getVoiceJarURLsFromDir(String dirName) throws FileNotFoundException 
    {
        try {
          ...
          if (DBUG_VOICE_LOADING) System.out.println
            ("[WARN] Security problem in getVoiceJarURLsFromDir()!\n"+se.getMessage());
          return null;
    ...
    private Voice[] createDefaultVoices() {
      Class c = com.sun.speech.freetts.en.us.cmu_us_kal.KevinVoiceDirectory.class;
      VoiceDirectory vd;
      try {
        vd = (VoiceDirectory)c.newInstance();
      } catch (Throwable e) {
        System.err.println("[WARN] Unable to create default voices! "+e.getMessage());
        return null;
      }
      return vd.getVoices();       
    }
     ...
     */
    public static ClassLoader getVoiceClassLoader() {
        return classLoader;
    }
}
