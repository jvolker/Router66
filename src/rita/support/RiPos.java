...
{  
 ...
 /* 
  public POS toWordnet() 
  {
    return toWordnet(this);
  }*/
  
  /** 
   * Retuns true if <code>pos</code> exists in the group <code>choices</code>
   * @invisible
   */
  ...
  private boolean in(RiPos[] choices)
  {
    for (int i = 0; i < choices.length; i++) 
      if (this == choices[i]) return true;
    return false;
  }
  
  private static boolean in(String pos, String[] choices)
  {
    for (int i = 0; i < choices.length; i++) 
      if (pos.equalsIgnoreCase(choices[i])) 
        return true;
    return false;
  }
