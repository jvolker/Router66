...
import rita.*;
import rita.support.dyn.*;
import rita.support.ifs.RiWordnetIF;

/** 
 * Dynamically loads the RiTa.WordNet library (if it is present)
 * and delegates calls to it. Generally used for accessing Wordnet
 * via the RiTaServer. Note: for most cases, is simpler to just use
 * the rita.wordnet.RiTaWordnet that comes with RiTa.WordNet pkg.
 * 
 * @invisible 
 */
...
System.out.println("Creating remote Wordnet instance...");
        delegate = (RiWordnetIF)RiDynamicRemote.implement(RiWordnetIF.class, this);
        //delegate = new RemoteWordnet(Class.forName(RITA_WORDNET_CLASS));
      }
      else {
System.out.println("Creating local Wordnet instance...");        
        delegate = RiWordnetImpl.getInstance();
      }
    }
    catch (ClassNotFoundException e) 
    {
      throw new RiTaException("Unable to load "+RITA_WORDNET_CLASS+"! Make sure that " +
        "you've installed\n          the RiTa.Wordnet library (" +RITA_WORDNET_URL+")");            		
    }
    //System.out.println("Wordnet="+delegate);
  }
  
  public static void main(String[] args) {
    RiTa.useServer();
    RiWordnetSupport ri = new RiWordnetSupport(null);  
    //System.out.println(RiTa.asList(ri.getSynonyms("dog", "n")));  
    //System.out.println(RiTa.asList(ri.getAntonyms("hot", "a")));
    //System.out.println(RiTa.asList(ri.getAllExamples("hot", "a")));
  }
