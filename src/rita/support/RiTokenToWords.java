 ...
 * words which are placed back into the Utterance. Usually, the tokens
 ...
 * </code>
 */
public class RiTokenToWords implements UtteranceProcessor
{
  ...
  private static final String[] kingNames = { "louis", "henry", "charles",
      "philip", "george", "edward", "pius", "william", "richard", "ptolemy",
      "john", "paul", "peter", "nicholas", "frederick", "james", "alfonso",
      "ivan", "napoleon", "leo", "gregory", "catherine", "alexandria",
      "pierre", "elizabeth", "mary" };

  private static final String[] kingTitles = { "king", "queen", "pope", "duke",
      "tsar", "emperor", "shah", "caesar", "duchess", "tsarina", "empress",
      "baron", "baroness", "sultan", "count", "countess" };

  // Section-like words
  private static final String[] sectionTypes = { "section", "chapter", "part",
      "phrase", "verse", "scene", "act", "book", "volume", "chap", "war",
      "apollo", "trek", "fortran" };
  ...
  private static final String[][] usStates = {
      ...
      { "Ala", "", "alabama" }, { "AK", "", "alaska" }, { "Ak", "", "alaska" },
      { "AZ", "", "arizona" }, { "Az", "", "arizona" },
      { "CA", "", "california" }, { "Ca", "", "california" },
      { "Cal", "ambiguous", "california" }, { "Calif", "", "california" },
      { "CO", "ambiguous", "colorado" }, { "Co", "ambiguous", "colorado" },
      { "Colo", "", "colorado" }, { "DC", "", "d", "c" },
      { "DE", "", "delaware" }, { "De", "ambiguous", "delaware" },
      ...
      { "IL", "ambiguous", "illinois" }, { "Il", "ambiguous", "illinois" },
      { "ILL", "ambiguous", "illinois" }, { "KS", "", "kansas" },
      { "Ks", "", "kansas" }, { "Kans", "", "kansas" },
      { "KY", "ambiguous", "kentucky" }, { "Ky", "ambiguous", "kentucky" },
      { "LA", "ambiguous", "louisiana" }, { "La", "ambiguous", "louisiana" },
      ...
      { "Mich", "ambiguous", "michigan" }, { "MN", "ambiguous", "minnestota" },
      ...
      { "WA", "ambiguous", "washington" }, { "Wa", "ambiguous", "washington" },
  ...
  private RiWordRelation wordRelation;
   ...
   */
  public RiTokenToWords(CART usNumbersCART, PronounceableFSM prefixFSM,
  ...
  {
//System.out.println("RiTokenToWords.processUtterance()");
    ...
    wordRelation = RiWordRelation.createWordRelation(utterance, this);
  ...
  private boolean matchesPartPhoneNumber(String tokenVal)
  {
    ...
    return ((matches(threeDigitsPattern, tokenVal) && ((!matches(digitsPattern,
        p_name)
        && matches(threeDigitsPattern, n_name) && matches(fourDigitsPattern,
        n_n_name))
   ...
   * 
   * @param tokenVal
   *          the String value of the token, which may or may not be same as the
   *          one in called "name" in flite
   * 
   */
  private void tokenToWords(String tokenVal)
  {
    ...
    boolean pho = tokenFeatures.isPresent("phones");
    if (tokenFeatures.isPresent("phones"))
    {
      wordRelation.addWord(tokenVal);
        ...
        && ((tokenItem.getNext() == null) || !(tokenVal.equals(itemName)) || !(((String) tokenItem
    ...
    else if (matches(alphabetPattern, tokenVal))
    {
      ...
      {
        /*
         * The name of a US state isStateName() has already added the full name
         * of the state, so we're all set.
         */
      } 
      else if (tokenLength > 1 && !isPronounceable(tokenVal))
      {
        ...
        RiNumberExpander.expandLetters(tokenVal, wordRelation);
    ...
    else if (matches(dottedAbbrevPattern, tokenVal))
    {
      ...
      RiNumberExpander.expandLetters(aaa, wordRelation);
    ...
    else if (matches(commaIntPattern, tokenVal))
    {
      ...
      /* 99,999,999 */
      String aaa = Utilities.deleteChar(tokenVal, ',');
      RiNumberExpander.expandReal(aaa, wordRelation);
    ...
    else if (matches(sevenPhoneNumberPattern, tokenVal))
    {
      ...
      RiNumberExpander.expandDigits(aaa, wordRelation);
      wordRelation.addBreak();
      RiNumberExpander.expandDigits(bbb, wordRelation);
    ...
    else if (matchesPartPhoneNumber(tokenVal))
    {
      ...
      RiNumberExpander.expandDigits(tokenVal, wordRelation);
      wordRelation.addBreak();
    ...
    else if (matches(numberTimePattern, tokenVal))
    {
      ...
      RiNumberExpander.expandNumber(aaa, wordRelation);
      if (!(bbb.equals("00")))
      {
        RiNumberExpander.expandID(bbb, wordRelation);
      }
    ...
    {
      RiNumberExpander.expandReal(tokenVal, wordRelation);
    }
    else if (matches(ordinalPattern, tokenVal))
    {
      /* explicit ordinals */
      String aaa = tokenVal.substring(0, tokenLength - 2);
      RiNumberExpander.expandOrdinal(aaa, wordRelation);
  ...
  private void digitsToWords(String tokenVal)
  {
    ...
    if (nsw.equals("nide"))
    {
      RiNumberExpander.expandID(tokenVal, wordRelation);
    } else
    {
      ...
      {
        RiNumberExpander.expandOrdinal(tokenVal, wordRelation);
      ...
      {
        RiNumberExpander.expandNumber(tokenVal, wordRelation);
      }
    }
  }
  ...
  private void romanToWords(String romanString)
  {
    ...
    if (punctuation.equals(""))
    {
      ...
      String n = String.valueOf(RiNumberExpander.expandRoman(romanString));

      if (kingLike(tokenItem))
      {
        wordRelation.addWord("the");
        RiNumberExpander.expandOrdinal(n, wordRelation);
      } else if (sectionLike(tokenItem))
      {
        RiNumberExpander.expandNumber(n, wordRelation);
      } else
      {
        RiNumberExpander.expandLetters(romanString, wordRelation);
      }
    } else
    {
      RiNumberExpander.expandLetters(romanString, wordRelation);
  ...
  /**
   * Returns true if the given key is in the kingSectionLikeHash Hashtable, and
   * the value is the same as the given value.
   * 
   * @param key
  ...
  private void drStToWords(String drStString)
  {
    ...
    } else
    {
      ...
      {
        wordRelation.addWord(street);
      } else if (RiNumberExpander.isDigit(p0) && isLowercaseLetter(n0))
      {
        wordRelation.addWord(street);
  ...
  private void usMoneyToWords(String tokenVal)
  {
    ...
    {
      RiNumberExpander.expandReal(tokenVal.substring(1), wordRelation);
    } else if (dotIndex == -1)
    {
    ...
    } else
    {
      ...
      {
        RiNumberExpander.expandNumber(bbb, wordRelation);
        if (bbb.equals("01"))
        {
          wordRelation.addWord("cent");
        } else
  ...
  private void digitsSlashDigitsToWords(String tokenVal)
  {
    ...
    {
      RiNumberExpander.expandNumber(aaa, wordRelation);
      RiNumberExpander.expandOrdinal(bbb, wordRelation);
      if (a > 1)
      {
        wordRelation.addWord("'s");
      }
  ...
  /**
   * Returns true if the given word is pronounceable. This method is originally
   * called us_aswd() in Flite 1.1.
   * 
   * @param word
   *          the word to test
   * 
    ...
    if (state != null)
    {
      ...
      if (state[1].equals("ambiguous"))
      {
        ...
        boolean previousIsCity = (isUppercaseLetter(previous.charAt(0))
            && previous.length() > 2 && matches(alphabetPattern, previous) && tokenItem
   ...
   * <p>
   * 1) the character and the following character are not letters in the English
  ...
  private static boolean isTextSplitable(String text, int index)
  {
    ...
    {
      return false;
    } else if (RiNumberExpander.isDigit(c0) && RiNumberExpander.isDigit(c1))
    {
      return false;
    } else
    {
