...
public class Definition
{
  ...
  public Definition(RuleParser scanner) {
    ...
    while (Grammar.isLineBreak(ruleName))
...
//System.out.println("Grammar.token='"+token+"'");
      ...
      if (token.startsWith(Grammar.OPEN_QUANT))
      {
        if (l.size() > 0)
          throw new RiTaException("[ERROR] Misplaced quantifier character: "+
            Grammar.OPEN_QUANT+"' or '"+Grammar.CLOSE_QUANT+"; quantifiers must " +
      ...
      // handle end of rule:  '|'
      if (token.equals(Grammar.END_PROD) || token.equals(Grammar.CLOSE_DEF))
      {                
        if (l.size() > 0) {
          addRule(l, multiplier);
        }
        return token.equals(Grammar.END_PROD); // only false on end-of-definition
      }
...
{   
    return (l.startsWith(Grammar.COMMENT));
  }
  ...
  private int handleMultiplier(String token)
  {
    ...
    if (!token.endsWith(Grammar.CLOSE_QUANT))
      throw new RiTaException("[ERROR] Misplaced quantifier character "+
          "('"+Grammar.OPEN_QUANT+"' or '"+Grammar.CLOSE_QUANT+"')" +
        " quantifiers must end with '"+ Grammar.CLOSE_QUANT +
        "' - at token:'... " + token + " ...'");
...
/*  private static boolean isMultiplier(String token)
  ...
  public void expand(Grammar grammar, StringBuilder StringBuilder)
  {
    if (rules.size() < 1)
...
//System.out.println("   "+toExpand);
    ...
    if (Grammar.DBUG_EXPAND) System.out.println("CHOSE: " + toExpand);
    ...
    while (e.hasNext())
    {
      ...
      if (prod.startsWith(Grammar.COMMENT)
          || prod.startsWith(Grammar.OPEN_QUANT))
        continue;
...
//System.out.println("Definition.expand() prod="+prod);
      ...
      if (prod.indexOf(Grammar.OPEN_TOKEN) >= 0)
