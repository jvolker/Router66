...
//System.out.println((count++)+") '"+result+"'");  
  ...
  {
    if (result != null) { 
      if (result.contains(Grammar.EXEC_CHAR)) {
        while (!result.contains(Grammar.EXEC_POST)) {
...
//System.out.print("RuleParser.preProcessExec("+result+")");
    int postIdx = result.indexOf(Grammar.EXEC_POST);
    if (postIdx < 0)
      throw new RiTaException("Unterminated exec call? string="+result);
    if (postIdx != result.length()- Grammar.EXEC_POST.length()) {
      //throw new RiTaException("Invalid exec call? string="+result);
      String pre = result.substring(0,postIdx+Grammar.EXEC_POST.length());
      String post = result.substring(postIdx+Grammar.EXEC_POST.length());
      result = pre; 
...
//System.out.println(" - > "+result);
  ...
  {    
    ...
    try {   
      ...
      throw new RiTaException(ie,ie.getMessage()+", token="+i);
    }
